export default async function handler(req, res) {
  ...
  async function zohoCall(url, options, label, retryWithoutDuplicates = true) {
    ...
    if (Array.isArray(data.data) && data.data[0]?.status === "error") {
      ...
      if (errorCode === "DUPLICATE_DATA" && retryWithoutDuplicates && options.method === "PUT") {
        ...
        console.warn(`${label} duplicate on ${conflictField} (conflicts with record ${conflictId}). Retrying without it...`);
  ...
  try {
    ...
    if (leadData.data && leadData.data.length > 0) {
      ...
      console.log("Lead not in Leads, checking Contacts...");
    ...
    // UPDATE LEAD - Email & Phone intentionally excluded to avoid Zoho duplicate-check conflicts.
    // The lead already has Email/Phone from the initial inquiry; updates to those fields
    // should be handled manually in Zoho if needed.
    if (leadId) {
    ...
    if (!contactData.data || contactData.data.length === 0) {
      ...
      // UPDATE CONTACT - Email & Phone intentionally excluded to avoid Zoho duplicate-check conflicts.
